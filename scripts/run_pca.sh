...
echo "[$(date)] Starting PCA" | tee -a "$LOG"
...
for EMB in $MODELS; do
    for Y in $VARS; do
        ...
        echo "[$(date)] === PCA $EMB x $Y ===" | tee -a "$LOG"
        python pca_full_prot.py "$TRAIN" "$TEST" --analysis pca 2>&1 | tee -a "$LOG" || echo "[$(date)] [WARN] PCA $EMB x $Y failed" | tee -a "$LOG"
        ...
        echo "[$(date)] $EMB x $Y PCA done" | tee -a "$LOG"
    done
done

echo "[$(date)] PCA done" | tee -a "$LOG"
