#!/bin/bash
# SHAP feature importance analysis for all 2G models × all tasks.
...
echo "[$(date)] Démarrage SHAP" | tee -a "$LOG"
...
for EMB in $MODELS; do
    ...
    for Y in $VARS; do
        ...
        echo "[$(date)] === SHAP $EMB × $Y ===" | tee -a "$LOG"
        python pca_full_prot.py "$TRAIN" "$TEST" --analysis shap 2>&1 | tee -a "$LOG" \
            || echo "[WARN] SHAP $EMB × $Y échec" | tee -a "$LOG"
...
echo "[$(date)] SHAP terminé" | tee -a "$LOG"
